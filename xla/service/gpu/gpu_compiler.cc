...
absl::Status GpuCompiler::OptimizeHloPostLayoutAssignment(
...

  {
    ...
    pipeline.AddPass<SubByteNormalization>(
        SubByteNormalization::SET_ELEMENT_SIZE);
    AddDeviceSpecificScratchSizePasses(&pipeline, gpu_target_config);
    RETURN_IF_ERROR(
        pipeline.Run(hlo_module, {HloInstruction::kMainExecutionThread})
            .status());
  }
...
absl::StatusOr<ScheduleMetadata> GpuCompiler::ScheduleAndVerify(
    HloModule* module, const GpuTopology& gpu_topology,
    const GpuAliasInfo* alias_info) {
  TF_RETURN_IF_ERROR(RunPreSchedulingPasses(
      module, gpu_topology.gpu_target_config().device_description, alias_info));
  TF_ASSIGN_OR_RETURN(
...
absl::StatusOr<std::vector<std::unique_ptr<Executable>>> GpuCompiler::Compile(
    std::unique_ptr<HloModule> hlo_module,
    std::vector<se::StreamExecutor*> stream_execs,
    const CompileOptions& options) {
  // HLO-level constant folding evaluates FP arithmetic on the host. Disable
  // FTZ/DAZ on this thread for the duration of compilation so TF's
  // process-wide setting doesn't leak into folded constants.
...
absl::Status GpuCompiler::AddConvAndGemmAutotuningPass(
  ...
  // Platforms with no autotunable codegen backends (e.g. Metal today)
  // register an empty factory; skipping the pass avoids
  // Autotuner::Create rejecting an empty backend list.
  if (backends.empty()) {
    return absl::OkStatus();
  }
...
absl::Status GpuCompiler::AddFusionAutotuningPass(
  ...
  // Metal has no Triton-equivalent block-level emitter to autotune against
  // the native MLIR emitter, and the autotuner's cache key doesn't yet
  // know how to format Metal's compute capability. Skip the pass until
  // there's actually a second fusion backend to choose between.
