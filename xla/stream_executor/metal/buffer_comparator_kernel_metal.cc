...
// infinities clamp to ±65504 before comparing; integers compare through
// float like the CUDA kernels. The 32-bit add matches CUDA; the host reads
// the count from a zero-initialized 8-byte buffer.
//
// $0 = kernel name, $1 = MSL element type, $2/$3 = elem_a/elem_b loads.
constexpr absl::string_view kComparisonMslTemplate = R"msl(
...
void RegisterComparator(absl::string_view name, absl::string_view msl_type,
  ...
  sources->push_back(absl::Substitute(
      kComparisonMslTemplate, name, msl_type,
  ...
  if (!result.ok()) {
    LOG(FATAL) << "Failed to register Metal buffer comparator kernel "
               << name << ": " << result;
  }
}
...
void RegisterBufferComparatorKernelMetalImpl() {
  ...
  RegisterComparator<Eigen::half>("f16_comparison", "half", true, false,
                                  false);
  ...
  RegisterComparator<uint16_t>("u16_comparison", "ushort", false, false,
                               true);
